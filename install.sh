...
#
# 編集対象は次のいずれか:
#   ${overlay_dir}/section/<section-name>.py   (section 単位の独自設定)
#   ${overlay_dir}/apps/<name>.py              (アプリ別の独自設定; section-base-2 経由で読まれる)
#
# 各 section の処理:
#   1. _config_personal.py (upstream pristine sample) の同 section を抜き出して exec
#   2. ${overlay_dir}/section/<section>.py を exec (存在すれば)
HEADER
...
def _fmx_custom(s):
    path = _p.join(r"${overlay_dir}/section", s + ".py")
    if not _p.exists(path): return
    with open(path, encoding="utf-8-sig") as f: src = f.read()
    exec(src, globals())
...
# --- 6. overlay/section/ に section 別の空ファイルを足す (既存はそのまま) ----
mkdir -p "$SELF_DIR/overlay/section"
mkdir -p "$SELF_DIR/overlay/apps"
while IFS= read -r section; do
    overlay_file="$SELF_DIR/overlay/section/${section}.py"
    if [ -e "$overlay_file" ]; then
        continue
    fi
    if [ "$section" = "section-base-2" ]; then
        # section-base-2 のデフォルト中身: overlay/apps/ 配下を一括ロード
        cat > "$overlay_file" <<APPS_LOADER
...
APPS_LOADER
    ...
    log "  created overlay : overlay/section/${section}.py"
...
log ""
log "編集対象:"
log "  $OVERLAY_DIR/section/<section-name>.py   (section 単位)"
log "  $OVERLAY_DIR/apps/<name>.py              (アプリ別; <name>.exe にマップ)"
